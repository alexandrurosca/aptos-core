...
module marketplace::marketplace_listing_utils {
    ...
    public fun direct_listing<CoinType>(
        owner: &signer,
        creator: address,
    ...
    public fun change_price_listing<CoinType>(
        ...
        let expiration_sec = timestamp::now_seconds() + 157680000; // 5 years
    ...
    fun initialize_listing_records<CoinType>(owner: &signer){
        let owner_addr = signer::address_of(owner);
    ...
    fun create_list_under_user_account<CoinType>(
        owner: &signer,
        token_id: TokenId,
        amount: u64,
        min_price: u64,
    ...
    fun create_listing_id_raw(lister: address, listing_creation_number: u64): ID {
        guid::create_id(lister, listing_creation_number)
    }

    fun get_listing_id<CoinType>(list: &Listing<CoinType>): ID {
        list.id
    }

    fun get_listing_id_tuple<CoinType>(list: &Listing<CoinType>): (u64, address) {
        let id = list.id;
        (guid::id_creation_num(&id), guid::id_creator_address(&id))
    }

    fun get_listing_creator<CoinType>(list: &Listing<CoinType>): address {
        guid::id_creator_address(&list.id)
    }

    fun get_listing_token_id<CoinType>(list: &Listing<CoinType>): TokenId {
        list.token_id
    }

    fun get_listing_expiration<CoinType>(list: &Listing<CoinType>): u64 {
        list.expiration_sec
    }

    fun get_listing_start<CoinType>(list: &Listing<CoinType>): u64 {
        list.start_sec
    }

    fun get_listing_min_price<CoinType>(list: &Listing<CoinType>): u64 {
        list.min_price
    }

    fun get_listing_token_amount<CoinType>(list: &Listing<CoinType>): u64 {
        list.amount
    }

    fun get_listing_instant_sale<CoinType>(list: &Listing<CoinType>): bool {
        list.instant_sale
    }

    fun create_listing_event(
        id: ID,
        token_id: TokenId,
        amount: u64,
        min_price: u64,
